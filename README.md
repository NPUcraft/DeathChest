...
> 当前项目尚未正式发布，不保证兼容任何旧版数据库或旧配置。测试升级时请先备份，并使用全新的插件数据目录或数据库。
...
构建产物为 `target/DeathChest-1.0.0.jar`，其中已经包含 SQLite 和 MySQL JDBC 驱动。将 JAR 放入服务器的 `plugins/` 目录并启动，默认生成：
...
- Recovery Storage 默认 30 天后删除未领取物品。
